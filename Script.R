...
# denne løsning er baseret på min egen tankegang, der kan være teori som er 
# fuldstændig forkert eller data der slet ikke passer i funktionen når i kører det
# er der problemer så spørg


# Dette er en opgave som går ud på at bruge noget af det vi har set før men også
# at søge på google for at lære noget nyt

data("ChickWeight")
head(ChickWeight)

# Vi vil gerne se hvilken indflydelse diet har på weight. Vi tror derfor at Diet
# skal laves om til en dummy variabel inden vi laver regressionen. 

iN <- length(unique(ChickWeight$Diet)) # forskellige værdier af diet

Dummy <- matrix(NA, 578, 4)

for (i in 1:iN){
  Dummy[, i] <- ifelse(ChickWeight$Diet == i, 1, 0)
}
colnames(Dummy) <- c("D1", "D2", "D3", "D4")
# nu har vi lavet "Dummy" nu skal den sættes sammen med vores data

# "how to connect two datasets in r"

?cbind

ChickWeight <- cbind(ChickWeight, Dummy)


summary(lm(ChickWeight$weight ~ ChickWeight$D2 + ChickWeight$D3 + ChickWeight$D4))

# "make a dummy variable from a factor in r"

install.packages("fastDummies")

?fastDummies::dummy_cols

ChickWeight <- fastDummies::dummy_cols(ChickWeight, "Diet")

summary(lm(ChickWeight$weight ~ ChickWeight$Diet_2 + ChickWeight$Diet_3 + ChickWeight$Diet_4))


# nu har vi kæmpet lidt med at lave dummies, men for at tjekke om vi selv er nogle
# dummies søger vi på google

# "regression with categorical variables in r"
# og finder ud af at R faktisk selv kan finde ud af det når vi har lavet det som en factor

summary(lm(ChickWeight$weight ~ ChickWeight$Diet))


# problemer opstår hvis vi ikke laver det om til en factor

class(ChickWeight$Diet) # dette er allerede en factor
class(ChickWeight$Time) # dette er "numeric" - tal


Diet_notfactor <- as.numeric(ChickWeight$Diet) #laver det til tal-format
summary(lm(ChickWeight$weight ~ Diet_notfactor)) #regressionen tror nu at data er 1, 2, 3, 4

# Vi kan altså se at forskellige datatyper kan bruges til forskellige ting, med
# meget lidt manipulation kan man gå imellem forskellige typer
...
rente <- read.csv("C:/Users/Lasse H. Krejberg/Desktop/R-kursus/rente.csv", header=FALSE)
inflation <- read.csv("C:/Users/Lasse H. Krejberg/Desktop/R-kursus/inflation.csv", header=FALSE)

# rense dataet en smule

inflation <- inflation[, -1]

# tjekke om datoerne passer

identical(inflation[, 1], rente[, 1])

# begge er nu i en data.frame. Husk dette. Vi skal lave en funktion der kun tager 
# rente og inflation og spytter noget ud til os

analysis <- function(vI, vPi, dates) {
  
  iT <- length(vI)
  iT_test <- length(vPi)
  
  if (identical(iT, iT_test) == FALSE) {
    cat("Vi har en fejl og funktionen stoppe her")
    
  }else{
    
  Real_rente <- (1 + vI) / (1 + vPi) - 1
  
  # i hvilke perioder er real_rente negative?
  
  index <- which(Real_rente < 0)  
  
  negativ_realrente <- dates[index]
  
  # check om tidsserierne er stationære med en regression
  
  reg_rente <- summary(lm(vI[2:218] ~  vI[1:217]))
  reg_infl <- summary(lm(vPi[2:218] ~  vPi[1:217]))
  
  # cointegration
  
  reg_co <- summary(lm(vI ~ vPi))
  
  residuals <- reg_co$residuals
  reg_res <- summary(lm(residuals[2:218] ~ residuals[1:217]))
  }
  
  lOut <- list()
  lOut[["Realrente"]] <- Real_rente
  lOut[["negativ_r"]] <- negativ_realrente 
  lOut[["reg_rente"]] <- reg_rente
  lOut[["reg_infl"]] <- reg_infl
  lOut[["reg_res"]] <- reg_res
